import numpy as np
from SIFmaster.src import SIF_embedding
...
def emb_weighted_average_sim_rmpc(emb1,emb2):
    """
    Compute the scores between pairs of sentences using weighted average + removing the projection on the first principal component
    :param emb1: the sentence embeddings pharse1
    :param emb2: the sentence embeddings pharse2
    :param params.rmpc: if >0, remove the projections of the sentence embeddings to their first principal component
    :return: scores, scores[i] is the matching score of the pair i
    """
    #emb1 = SIF_embedding.SIF_embedding(We, x1, w1, params)
    #emb2 = SIF_embedding.SIF_embedding(We, x2, w2, params)
    #print(np.shape(emb1*emb2))
    inn = (emb1 * emb2).sum()
