...
def read_similarity_score(lda,lsi,vsm):
    """Read an IR model and align it with source and target ids.
  Args:
    lda,lsi,vsm: IR models to align.
  Returns:
...
def buildingTrainingSet(lsi_align, vsm_align, lda_align,matrixSize=3):
  ...
  """         
    ...
    #building the description matrix
    matrix_values=building_aggreg_Matrix(lsi_align, vsm_align, lda_align,  matrixSize)
    ...
    print(len(falseL))
    print(len(trueL))
    print("truelinks and false links created")
...
def compute_aggreg (pred_grd, irmodel_align, filename ) :
  ...
  Returns:
    ...
    with open(filename, 'w') as csvfile:
        ...
        for i in range(0,len(irmodel_align)):
                writer.writerow({'Artifact1': str("{0}".format(irmodel_align[i][0])), 'Artifact2': str("{0}".format(irmodel_align[i][1])), 'probability': str("{0}".format(pred_grd[i]))})  
       
    
    print("similarity matrix build")
    
    
def aggreg(lda,lsi,vsm,filename):
  ...
  Returns:
    ...
    #we fixe the randow matrix at 50000 lines
    numLine=50000
    
    lsi_align, vsm_align, lda_align =read_similarity_score(lda,lsi,vsm)  
    vect,matrix_values = buildingTrainingSet(lsi_align, vsm_align, lda_align)
